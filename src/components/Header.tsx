      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20 gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            {/* Role Switcher (Khách / Quản trị viên Admin) */}
            <button
              id="role-switch-btn"
              ...
              title={
                userRole === 'admin'
                  ? 'Đang ở chế độ Admin: Có toàn quyền Upload PDF & quản trị. Bấm để chuyển sang chế độ Khách xem.'
                  : 'Đang ở chế độ Khách (Học sinh/Giáo viên): Xem và tải tài liệu. Bấm để đăng nhập vai trò Admin.'
              }
            >
              {userRole === 'admin' ? (
                <>
                  ...
                  <span>Admin</span>
                  <span className="text-[10px] bg-slate-900 text-amber-300 px-1.5 py-0.2 rounded-full font-extrabold">
                    Quản trị
                  </span>
                </>
              ) : (
            ...
            {/* Admin Upload Button (Visible to Admin, or prompt to switch) */}
            {userRole === 'admin' ? (
              <button
                id="open-admin-upload-btn"
                onClick={onOpenAdminUpload}
                className="px-3.5 py-2 rounded-xl text-xs font-bold text-white bg-blue-700 hover:bg-blue-800 active:bg-blue-900 transition-all flex items-center gap-1.5 shadow-sm shadow-blue-700/20 cursor-pointer"
              >
              ...
              <button
                id="guest-request-admin-btn"
                onClick={onToggleUserRole}
                className="px-3 py-2 rounded-xl text-xs font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 transition-colors hidden sm:flex items-center gap-1.5"
                title="Bấm để kích hoạt quyền Quản trị viên tải tài liệu mới"
              >
